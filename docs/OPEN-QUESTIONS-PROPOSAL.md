...
- **Status (revisi 28 Agu 2026):** ❌ Keputusan 20 Agustus DIBATALKAN — premisnya salah. Dicek langsung ke fonnte.com: **Fonnte SENDIRI adalah layanan "unofficial"** ("Layanan fonnte merupakan layanan unofficial", FAQ resmi mereka), jalan di atas WhatsApp Web otomatis (mekanisme SAMA persis dengan `whatsapp-web.js`), BUKAN WhatsApp Business API resmi. FAQ Fonnte eksplisit bilang: "Apa ada resiko banned menggunakan layanan ini? ... resiko banned tetap ada", dan "Apakah dengan berlangganan bisa membuat device lebih aman dari banned? Tidak. peluang terkena banned sama baik gratis maupun berbayar." Kolom "Risiko: Rendah" di tabel trade-off di atas **tidak akurat** — risiko banned Fonnte dan `whatsapp-web.js` pada dasarnya SAMA, bedanya cuma siapa yang maintain infrastruktur koneksinya (Fonnte host-kan sendiri, `whatsapp-web.js` self-hosted).
- **Keputusan baru (28 Agu 2026):** pakai **`whatsapp-web.js` self-hosted**, gratis, dan Fakrul sudah pengalaman pakai di project Hermes. Karena risiko banned toh sama, tidak ada alasan bayar Fonnte/Wablas hanya demi kemudahan infrastruktur — biaya bisa dialokasikan ke hal lain.
- **Arsitektur integrasi:** proses Node.js kecil (`whatsapp-web.js` dibungkus Express/HTTP server lokal, misal port 3001) jalan terpisah 24/7 di samping `php artisan serve`. Laravel kirim notifikasi via `Http::post()` ke endpoint lokal itu — Laravel sendiri TIDAK perlu tahu detail WhatsApp Web/Puppeteer, cukup panggil HTTP endpoint seperti manggil API pihak ketiga biasa.
- **Mitigasi risiko banned yang WAJIB tetap masuk desain** (berlaku independen dari provider mana pun dipakai):
  ...
  3. Volume rendah, bukan broadcast — kirim notifikasi satu-satu per event (apply/hasil seleksi/reminder), bukan blast massal ke banyak nomor sekaligus dalam waktu singkat.
  4. Kalau nomor ke-banned, sistem harus tetap jalan penuh lewat email (WA cuma tambahan) — jangan bikin fitur kritis bergantung ke WA doang.
...
- **Status:** ✅ RESOLVED & BUILT — masuk MVP, diimplementasi sebagai soft-warning 2 checkpoint (saat apply, Sprint 2; saat present ke CD/RF-22, Session 5 setelah checkpoint kedua sempat kelewat). Lihat `DEV-NOTES.md`.
...
- Ini **menggantikan** RF-17 s/d RF-20 di baseline lama. `CLAUDE.md` §6 (state machine) & §9 (backlog) sudah selaras dengan keputusan ini — dikonfirmasi 29 Agu 2026.
