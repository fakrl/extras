...
## Session 19 — Audit Full-Codebase (Correctness/Security + Ponytail) (30 Agustus 2026, malam — Fakrul tidur)

> **⚠️ BACA INI DULUAN, DI LUAR SCOPE AUDIT KODE: repo GitHub `fakrl/extras` PUBLIC** (dicek langsung: `curl` unauthenticated ke GitHub API balikin HTTP 200 — repo private bakal 404). `docs/DEV-NOTES.md` (file ini sendiri) dan `docs/CLAUDE.local.md` SAMA-SAMA punya catatan di baris pembuka masing-masing "file ini tidak masuk git (gitignore)" — tapi keduanya ke-track & ke-push sejak Session 2 (`5e2023e`), dan saya ikut push lagi beberapa kali semalam tanpa sadar sampai baru ketahuan barusan. Tidak ada credential/secret yang bocor (dicek: `.env` asli nggak pernah ke-track, `NIK_HASH_KEY`/`WHATSAPP_SERVICE_TOKEN` dkk cuma ada di `.env` yang gitignored) — tapi isi kedua file ini (nama tim, detail bisnis internal JBTB, catatan proses terus terang) sudah public dari awal, bukan kebocoran baru. Saya sengaja TIDAK utak-atik `.gitignore`/riwayat commit/visibility repo malam ini — itu keputusan Fakrul (ada tradeoff: portofolio publik vs privasi, kebutuhan dokumentasi akademik 8x bimbingan). Kalau mau ditutup: opsi ringan (`.gitignore` + `git rm --cached`, riwayat lama tetap nyimpen isi lama) atau opsi penuh (repo di-private-kan di GitHub settings, atau history rewrite kalau mau riwayat lama juga bersih — yang terakhir butuh force-push, hati-hati).

Fakrul minta lanjut kerja semalaman sambil dia tidur ("segala pertanyaan ditanyain sekarang" sudah dijawab sebelum tidur — SPEC.md batch terakhir tidak ada pertanyaan). Setelah SPEC.md kosong (tidak ada task baru tertulis), saya inisiatif jalankan 2 audit menyeluruh (bukan per-ticket-diff seperti biasa, tapi seluruh codebase sekaligus — belum pernah dilakukan sejak 18 sesi lalu): ponytail-audit (over-engineering) + review korektnes/keamanan penuh.

### Yang SUDAH diperbaiki (mekanis, sudah diverifikasi aman, tidak perlu keputusan Fakrul)

**1. 6 model nulis kolom `*_at` yang nggak ada di `#[Fillable]` — MassAssignmentException di dev, silent data loss di production.** Sama persis pola bug yang sudah 3x kejadian sebelumnya (User, FeeNegotiation, ExtrasProfile). Ditemukan: `Payment::tandaiDitransfer()`/`konfirmasiDiterima()` (`ditransfer_at`/`dikonfirmasi_at`), `ContractController::sign()` (`signed_at`), `AdminProjectAssignment::tandaiSelesai()` (`completed_at`), `StaffPayroll::tandaiSlipDibuat()` (`generated_at`), `AdminProfile::updateHonor()` (`honor_updated_at`). **Diverifikasi aman sebelum fix**: keenamnya nilai `now()` atau parameter typed yang sudah tervalidasi di controller, tidak ada satupun raw request passthrough — sama standar verifikasi yang dipakai 3 fix sebelumnya. Fix: tambah keenam kolom ke `#[Fillable]` masing-masing model. Test baru `tests/Feature/FillableTimestampRegressionTest.php` (6 test, satu per method) — dibuktikan REAL dengan revert-sementara `Payment::$fillable` dan konfirmasi 2 test gagal seperti yang diharapkan, baru dikembalikan. **Ini bug yang bikin fitur inti (tandai transfer, konfirmasi terima, TTD kontrak, tandai proyek selesai, update honor) 500 total di dev** — kenapa nggak ketahuan dari Sprint 1-6: nol test coverage untuk keenam method ini sampai sekarang.

**2. Margin/budget client bocor ke Extras** — `resources/views/extras/projects/index.blade.php` (label "Fee tertinggi") dan `show.blade.php` (kolom "Budget") menampilkan `budget_client` (fee dari client) langsung ke Extras. Pelanggaran tembok visibilitas `CLAUDE.md` §5 (budget_client = admin-only) — dan labelnya sendiri salah ("Fee tertinggi" padahal itu budget client, bukan fee yang bakal diterima Extras). **Fix**: index.blade.php ganti jadi "N kelas dibuka" (nggak ada angka uang). show.blade.php ganti kolom "Budget" jadi "Kriteria" — field `kriteria` (array, `CastingProjectClass`) sudah ada di model tapi TIDAK PERNAH dikumpulkan lewat form admin manapun (gap terpisah, dicatat di bawah) dan sudah dipakai dengan benar di bagian lain file yang sama (`show.blade.php` baris pemilihan kelas), jadi pola render-nya konsisten. Test baru `tests/Feature/ExtrasBudgetVisibilityTest.php` (2 test) — dibuktikan REAL dengan `git stash` sementara ke view lama, konfirmasi kedua test gagal, baru dikembalikan.

**3. WhatsApp dispatch bisa gagal DI LUAR try/catch yang ada** — `WhatsAppService::kirimNotifikasi()` manggil `SendWhatsAppNotification::dispatch()` tanpa guard; kalau insert ke tabel `jobs` sendiri gagal (DB lock/down), exception lolos ke pemanggil (mis. `Cd\ReviewController::review()` di tengah loop bulk-approve — sisa kandidat dalam batch yang sama nggak sempat diproses, CD lihat halaman error tanpa tau kandidat mana yang sudah/belum). Try/catch lama cuma ada SATU LAPIS TERLALU DALAM (di dalam `kirim()`/job `handle()`, yang keduanya baru jalan setelah dispatch sukses). **Fix**: bungkus `dispatch()` sendiri, catat gagal ke `NotificationLog` kalau exception. Satu titik perbaikan (`WhatsAppService::kirimNotifikasi()`) otomatis menutup celah di semua 4 titik panggil (hasil seleksi, konfirmasi apply, kontrak siap TTD, reminder H-1). Test baru `tests/Feature/WhatsAppDispatchFailureTest.php` — mock `Illuminate\Contracts\Bus\Dispatcher` supaya `dispatch()` sendiri throw, buktikan `kirimNotifikasi()` tetap tidak melempar exception ke pemanggil. Dibuktikan REAL dengan `git stash` sementara ke kode lama, konfirmasi test gagal dengan exception yang sama persis, baru dikembalikan.

**Hasil**: `php artisan test` — 87 (Session 18) → **96 passed**, 0 regresi. `./vendor/bin/pint`: passed.

### BUTUH KEPUTUSAN FAKRUL (tidak disentuh, catatan lengkap di bawah — cek satu-satu pas bangun)

Ini semua bug/gap NYATA (bukan cuma "consider checking"), tapi masing-masing butuh keputusan bisnis/scope yang bukan wewenang saya putuskan sendiri:

- **Akun nonaktif (`users.status`) & status `melanggar` (`ExtrasProfile.status`) ditulis tapi TIDAK PERNAH dibaca sebagai gate.** Extras yang di-nonaktifkan Admin tetap bisa login dan pakai sistem normal — satu-satunya efek nonaktif cuma label di tabel + angka di chart. Sama untuk status "Melanggar" (RF-08, 3x cancel mendadak). **Perlu keputusan**: block login sepenuhnya, atau cuma block aksi tertentu (apply lowongan baru misalnya, tapi tetap bisa lihat riwayat)? `Auth\LoginController.php`/`CheckRole.php` perlu ditambah gate begitu keputusannya jelas.

- **State machine pembayaran & partisipasi tidak terkait** — aplikasi `PaymentController` bisa jalan dari status APAPUN (bahkan `diajukan`, belum lewat nego/CD/kontrak sama sekali), dan `konfirmasi()` otomatis set `status_partisipasi = 'selesai_produksi'` tanpa cek kontrak ada/sudah ditandatangani. Efek samping: RF-30 rekap margin ikut over-report margin untuk aplikasi begini (payout 0 tapi `total_fee_client` tetap kehitung penuh). **Perlu keputusan**: guard status apa yang wajib sebelum `PaymentController::show()`/`tandaiTransfer()` boleh jalan (`lolos`+? harus ada kontrak signed?).

- **Nego fee bisa dibuka lagi SETELAH kontrak ditandatangani**, fee_final bisa ditimpa, dan `ContractController::sign()` akan regenerate PDF kontrak dengan angka baru — menimpa PDF lama yang sudah ditandatangani kedua pihak. Guard status di `Admin\FeeNegotiationController`/`Extras\FeeNegotiationController` daftar 5 status blokir tapi lupa masukin `kontrak_ditandatangani`/`selesai_produksi`/`dibatalkan`. **Ini kemungkinan besar cukup jelas untuk di-fix langsung** (tambah 3 status ke blocklist yang sudah ada) — tapi saya tahan karena nyentuh kontrak/tanda tangan (§14.1), plus keputusan tambahan: apa `ContractController::sign()` juga perlu guard "jangan re-sign kalau sudah `isFullySigned()`"?

- **Casting Director bisa akses invoice proyek mana pun** (`InvoiceController` cuma cek role, nggak cek CD itu terkait proyek yang mana) **dan approve/reject kandidat proyek mana pun** (`Cd\ReviewController` nggak filter per-CD/per-proyek sama sekali). Root cause-nya di level skema: `casting_projects` nggak punya kolom `cd_id`/tabel assignment CD↔proyek. **Perlu keputusan desain**: apa memang cuma ada 1 CD aktif per waktu (jadi ini bukan masalah), atau butuh model assignment CD↔proyek beneran (mirip `AdminProjectAssignment` yang sudah ada buat sub-role Admin)? Ini bukan bug kecil kalau JBTB memang mengelola banyak client bersamaan.

- **RF-08 (3x cancel mendadak → Melanggar) nyaris nggak pernah bisa kejadian** — syaratnya status HARUS `deal` (`ProjectApplication::batalkan()`), tapi "mendadak" (H-2 shooting) baru relevan setelah lewat CD+kontrak (status `lolos`/`kontrak_ditandatangani`), bukan `deal`. Dua kondisi ini nyaris saling meniadakan kecuali proyek "Butuh Dadakan". **Perlu keputusan**: perluas status yang boleh `batalkan()` ke `lolos`/`kontrak_ditandatangani` juga, atau itu memang di luar scope RF-33's original wording?

- **`nama_asli` (nama KTP) dikumpulkan encrypted tapi TIDAK PERNAH dipakai** — form `lengkapiKtp()` cuma minta NIK+rekening, tidak ada input nama asli sama sekali. PDF kontrak (`contracts/pdf-template.blade.php`) pakai alias sebagai nama penandatangan, bukan nama asli KTP — padahal `CLAUDE.md` §5 eksplisit bilang "begitu deal → Talent Release ditandatangani, nama asli (KTP) muncul di dokumen legal". **Perlu keputusan**: apa Talent Release beneran butuh nama asli (isu legalitas dokumen), dan kalau iya, tambah field di form `lengkapiKtp()`?

- **RF-35 (catatan Korlap): Korlap punya izin POST tapi nggak ada halaman yang bisa dia akses buat sampai ke situ.** Route `admin.applications.catatan` (POST) buka buat `admin_default,admin_korlap`, tapi satu-satunya halaman yang render form-nya (`admin.projects.applicants`, GET) ada di grup route `role:admin_default` MURNI. Nggak cuma soal buka route GET-nya — halaman itu juga nampilin tombol Grade/Reject/Nego/Batalkan tanpa ada pembatas visual per-role sama sekali (cuma section catatan yang sudah di-`@if` role), jadi kalau langsung buka akses GET-nya, Korlap bakal lihat tombol-tombol yang nanti 403 pas diklik. **Perlu keputusan desain**: apa yang boleh Korlap lihat di halaman itu selain form catatan (semua data pendaftar? cuma nama+foto+form catatan?) — bukan fix satu baris, butuh kerjaan Blade conditional tambahan setelah jelas scope-nya.

- **`docs/DEV-NOTES.md` sendiri ternyata TIDAK di-gitignore** meski komentar di baris 3 file ini bilang "di-gitignore (kalau belum, tambahkan), tidak ikut push ke repo" — sudah ke-commit & ke-push sejak Session 2 (`5e2023e`), termasuk sesi ini. Bukan saya yang mulai, tapi saya juga nggak berhenti (ikut push beberapa kali malam ini). **Perlu keputusan**: apa memang oke DEV-NOTES ini publik di GitHub (isinya cukup terus terang soal bug/reasoning internal), atau perlu di-`.gitignore`-kan + `git rm --cached` (itu sendiri langkah yang perlu hati-hati, riwayat commit lama tetap nyimpen isinya kecuali di-rewrite).

### Ponytail-audit (over-engineering, terpisah dari temuan bug di atas)

Audit kedua (fokus dead code/duplikasi, bukan bug) — semua temuan AMAN untuk di-refactor tanpa keputusan bisnis, tapi belum saya kerjakan malam ini (waktu habis buat bug-fix di atas dulu, yang lebih prioritas). Ringkasan buat sesi berikutnya:
- 2 pasang method byte-identik (`pastikanBolehLihat()` di `ContractController`/`PaymentController`; `pastikanMasihBisaNego()` di kedua `FeeNegotiationController`) — satukan jadi satu method di model.
- Status list (`['deal','diajukan_ke_cd',...]` dan `['lolos','kontrak_ditandatangani','selesai_produksi']`) di-copy manual 3x masing-masing di file berbeda — jadikan konstanta di `ProjectApplication`.
- 3 selector CSS mati (`.badge-status-aktif/-pending/-tolak`) di `layouts/app.blade.php` — tidak dipakai class manapun.
- `ExtrasProfile::simpanFoto()`/`simpanVideo()` identik strukturnya — bisa satu method `simpanMedia()`.
- `nik_hash` di-hitung ulang manual di `lengkapiKtp()` padahal mutator `nik()` udah hitung yang sama beberapa baris di bawahnya.
...
### Lanjutan sesi ini (masih malam yang sama) — tutup gap coverage "non-blocking" yang menumpuk dari sesi 1-19

Fakrul tidur, disuruh lanjut otomatis semalaman murni nulis test (nol perubahan kode produksi) — nyisir semua sesi lama buat gap yang tester pernah tandai "non-blocking, worth adding later" tapi nggak pernah ditutup. Tiap test baru dibuktikan REAL: revert sementara behavior yang diuji, konfirmasi test gagal, baru dikembalikan — sama seperti pola pembuktian 3 test Session 19 di atas.

1. **H-2 boundary tepat 2 hari (`batalkan()` RF-33/34):** `ProjectApplicationTest` sebelumnya cuma punya kasus longgar (`addDay()` = mendadak, `addDays(10)` = tidak) — belum ada kasus **tepat** di batas `< 2`. Tambah `test_batalkan_tepat_h2_dua_hari_tidak_mendadak` (shooting H+2 → `is_mendadak` harus `false`). Dibuktikan: ganti sementara `< 2` jadi `<= 2` di `ProjectApplication::batalkan()`, test gagal seperti diharapkan, dikembalikan.
2. **Otorisasi HTTP-level untuk `batalkan()` (RF-33/34):** sebelumnya cuma dites lewat panggilan method model langsung — rute `extras.negotiations.batalkan`/`admin.negotiations.batalkan` sendiri belum pernah kena HTTP request beneran di test manapun. File baru `tests/Feature/BatalkanAuthorizationTest.php` (6 test): Extras lain (bukan pemilik) → 403, Extras pemilik → berhasil, 3 sub-role Admin (talco/korlap/sosmed) → 403 (data provider), Admin Default → berhasil. Dibuktikan: (a) hapus sementara `pastikanMilikSendiri()` di `Extras\FeeNegotiationController::batalkan()` → test Extras-lain gagal seperti diharapkan; (b) pindahkan sementara route `admin.negotiations.batalkan` keluar dari grup middleware `role:admin_default` ke grup admin umum → ketiga test sub-role gagal seperti diharapkan. Keduanya dikembalikan.
3. **`nik_hash` konvergen lintas format NIK yang sama:** tester lama pernah flag test duplikat NIK cuma bandingin 2 NIK **berbeda**, bukan 1 NIK sama dalam format beda. Tambah 2 test di `LengkapiKtpTest.php`: (a) `test_nik_format_berbeda_untuk_digit_sama_hasilkan_hash_identik` — 3 instance `ExtrasProfile` in-memory (tidak disimpan ke DB, `nik_hash` UNIQUE) diisi NIK sama persis dalam 3 format (polos/strip/titik), assert `nik_hash` identik; (b) `test_nik_format_berbeda_dari_nik_terpakai_tetap_terdeteksi_duplikat` — NIK yang sudah dipakai profil lain, dicoba lagi dengan format berbeda (strip), harus tetap kena `NikDuplikatException`. Dibuktikan: (a) ganti sementara hash mutator `nik()` pakai `$value` mentah (bukan `$digits` yang sudah di-strip) → test (a) gagal; (b) ganti sementara hash-check di `lengkapiKtp()` pakai `$nik` mentah (bukan hasil `preg_replace`) → test (b) gagal (exception salah, `UniqueConstraintViolationException` bukan `NikDuplikatException`, TOCTOU F6/F8 Session 11 balik muncul). Keduanya dikembalikan.
4. **WhatsApp job HTTP-failure di level `handle()`:** test lama cuma cover nomor_wa null (skip) dan `dispatch()` sendiri gagal (`WhatsAppDispatchFailureTest`) — belum ada yang langsung instantiate `SendWhatsAppNotification` dan panggil `handle()` dengan HTTP call yang gagal (nomor_wa ADA, tapi Node service response 500). Tambah `test_job_handle_mencatat_gagal_saat_http_ke_node_gagal` di `WhatsAppNotificationTest.php`. Dibuktikan: ganti sementara `NotificationLog::catat(..., $terkirim, ...)` jadi hardcode `true` di `handle()` → test gagal seperti diharapkan, dikembalikan.
5. **Gap lain ditemukan saat nyisir DEV-NOTES (Session 7):** fix timezone UTC→Asia/Jakarta (`config/app.php`) sendiri sudah dicatat eksplisit "tidak ada test yang benar-benar membuktikan fix ini... catat sebagai potential follow-up". Tambah `test_batalkan_klasifikasi_mendadak_benar_saat_dini_hari_wib` di `ProjectApplicationTest.php` — bekukan waktu ke 01:00 WIB (`Carbon::setTestNow`, `Illuminate\Support\Carbon`), tanggal shooting H+1-versi-WIB tapi H+2-versi-UTC, assert `is_mendadak` tetap benar (`true`). Dibuktikan: ganti sementara default `config('app.timezone')` balik ke `'UTC'` → test gagal seperti diharapkan (`is_mendadak` jadi `false` karena "hari ini" kehitung mundur 1 hari), dikembalikan.
6. **Gap yang DICEK tapi TIDAK ditambah test:** Session 9 (`php artisan test: 32 passed... gap coverage untuk job ini sudah tercover test existing lewat sync driver`) — diverifikasi ulang, klaim itu benar untuk jalur SUKSES (job jalan in-process via `QUEUE_CONNECTION=sync`), tapi tidak untuk jalur GAGAL di level job — itu sebabnya poin 4 di atas tetap ditambahkan sebagai gap nyata, bukan diduplikasi.

**Hasil akhir:** `php artisan test` — 96 (checkpoint sebelumnya) → **107 passed** (0 gagal, 243 assertions), semua 11 test baru dibuktikan REAL (fail saat behavior direvert, pass saat dikembalikan). `./vendor/bin/pint`: passed (auto-fix minor di `LengkapiKtpTest.php` — urutan import + fully-qualify exception, tidak ada perubahan logic). **Tidak ada file `app/`/`routes/`/`resources/` yang berubah permanen** — semua revert sementara untuk pembuktian dikembalikan persis, dicek `git diff` kosong untuk keenamnya sebelum lanjut.

**File baru/berubah (lanjutan sesi ini):** `tests/Feature/BatalkanAuthorizationTest.php` (baru), `tests/Feature/ProjectApplicationTest.php`, `tests/Feature/LengkapiKtpTest.php`, `tests/Feature/WhatsAppNotificationTest.php`.
